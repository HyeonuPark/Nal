...
named!(parse_if_stmt(Input) -> Span<Stmt>, span!(map!(
    tuple!(
        separated_list!(
...
named!(parse_while_stmt(Input) -> Span<Stmt>, span!(map!(
    tuple!(
...
named!(parse_function_stmt(Input) -> Span<Stmt>, span!(map!(
    tuple!(
...
named!(parse_let_stmt(Input) -> Span<Stmt>, span!(map!(
    tuple!(word!("let"), sp, parse_pattern, sp, tag!("="), sp, parse_expr),
    |(_, _, pat, _, _, _, expr)| Stmt::Let(pat, expr)
)));

named!(parse_assign_stmt(Input) -> Span<Stmt>, span!(map!(
    tuple!(parse_pattern, sp, tag!("="), sp, parse_expr),
    |(pat, _, _, _, expr)| Stmt::Assign(pat, expr)
)));

named!(pub parse_stmt(Input) -> Span<Stmt>, alt_complete!(
    parse_if_stmt
    | parse_while_stmt
    | parse_function_stmt
    | parse_let_stmt
    | parse_assign_stmt
    | span!(map!(parse_expr, Stmt::Expr))
));
